...
const makeUtils = (build, keyword, table, parentTable, allowInherit, relevantRelation) => {
    ...
    const rawLocalDetails = expression
        ? appliesToTable(relevantClass)
            ? {
                expression: (_sql, tableAlias) => sql.fragment `(${expression(sql, tableAlias, { pgClass: relevantClass })})`,
    ...
    const parentDetails = appliesToTable(parentTable)
        ? expression
            ? {
                expression: (_sql, tableAlias) => sql.fragment `(${expression(sql, tableAlias, { pgClass: parentTable })})`,
                visibleFragment: booleanVisibleFragment,
                invisibleFragment: booleanInvisibleFragment,
            }
